...
const page = {
    ...
    header: {
        ...
        progressConverBar: document.querySelector('.progress__cover-bar'),
    },
    content: {
...
function rerenderHead(activeHabbit) {
    ...
    page.header.progressConverBar.style.width = `${progress}%`
}
...
function rerenderContent(activeHabbit) {
    ...
    for (const index in activeHabbit.days) {
        ...
        element.innerHTML = `<div class="habbit">
            <div class="habbit__day">Day ${Number(index) + 1}</div>
            <div class="habbit__comment">${activeHabbit.days[index].comment}</div>
            <button class="habbit__delete" onclick="deleteDay(${index})"><img src="./images/delete.svg" alt="delete day ${index + 1}"></button>
        </div>`
...
function setIcon(context, icon) {
    ...
    const activeIcon = document.querySelector('.icon-select__item.icon-select__item_active')
    activeIcon.classList.remove('icon-select__item_active')
    context.classList.add('icon-select__item_active')
}
